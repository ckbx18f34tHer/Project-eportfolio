...
from noise_delete.configs import AUDIO_PROCESSING_CONFIG, REALTIME_CONFIG, DEVICE_CONFIG
...
class NoiseReducer:
    ...
    def _process_audio(self, audio_tensor):
        ...
        if len(audio_tensor) < self.n_fft:
            self.logger.warning(f"输入音频过短 ({len(audio_tensor)} samples), 补零至 {self.n_fft}")
        ...
        if self._sr:
            ...
            if cutoff_bin > 1:
                high_pass_filter[:cutoff_bin, :] = torch.linspace(0.1, 1.0, cutoff_bin).unsqueeze(1)
            
            enhanced_spec = enhanced_spec * high_pass_filter
